...
- `structured`:
  - keeps module/file structure from `src-aot` in `www/dist`
  - sourcemaps enabled
  - best for debugging and inspecting generated files
- `bundle`:
...
Use structured build while coding:
...
After `npm run build:structured`, `gtpl-app-build` also writes `www/dist/importmap.json`. If your app adds dependencies, copy the generated mappings into the inline import map in `www/index.html`.
...
```bash
npx gtpl-aot
npx gtpl-app-build --mode structured
...
Options:
- `--mode structured`: modular output with sourcemaps.
...
Modes:
- `--mode structured`: modular output with sourcemaps, easier debugging.
